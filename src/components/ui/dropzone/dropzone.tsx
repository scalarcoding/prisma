...
const DropZone: React.FC<DropZoneReusableProps> = ({
  title,
  id,
  fileTypes,
  file,
  ...
  return (
    <div className="dropzone__container">
      ...
      <div
        className={`flex bg-white items-center justify-center h-8 w-8 ${
          isDragOver
            ? 'bg-blue-100 dark:bg-blue-800 border-blue-400 dark:border-blue-600'
            : 'bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600'
        } border-2 p-10 border-dashed rounded-lg cursor-pointer`}
        onDrop={handleDrop}
          ...
          <div className="flex flex-col items-center justify-center p-10">
            {uploadedFile ? renderFilePreview(uploadedFile) : (
              <div className='flex flex-col unloaded items-center'>
                <svg
                  className="w-8 h-8 text-slate-400 dark:text-gray-200"
                  aria-hidden="true"
